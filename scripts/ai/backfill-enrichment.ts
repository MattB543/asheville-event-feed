 ...
 *
 * Usage:
 *   npx tsx scripts/ai/backfill-enrichment.ts              # Process all events needing enrichment
 *   npx tsx scripts/ai/backfill-enrichment.ts --limit 10   # Process only 10 events
 *   npx tsx scripts/ai/backfill-enrichment.ts --dry-run    # Preview without saving
 *   npx tsx scripts/ai/backfill-enrichment.ts --source EXPLORE_ASHEVILLE  # Only one source
 */
...
import { tryExtractAndApplyTime } from '@/lib/utils/parsers';
