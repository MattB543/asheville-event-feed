...
const getCachedMetadata = unstable_cache(async () => getEventMetadata(), ['events-metadata'], {
  tags: ['events'],
  revalidate: 3600,
});
...
const PRICE_FILTERS = new Set<PriceFilter>(['any', 'free', 'under20', 'under100', 'custom']);

type TimeFilter = NonNullable<EventFilterParams['times']>[number];

const TIME_FILTERS = new Set<TimeFilter>(['morning', 'afternoon', 'evening']);
...
function parseTimes(value: unknown): EventFilterParams['times'] | undefined {
  ...
  const times = value.filter((time): time is TimeFilter => TIME_FILTERS.has(time as TimeFilter));
...
export async function GET(request: NextRequest) {
  try {
    ...
    const params: EventFilterParams = {
      ...
      priceFilter:
        (searchParams.get('priceFilter') as EventFilterParams['priceFilter']) || undefined,
    ...
    if (hasActiveFilters) {
      console.log(
        `[API /events GET] Query returned ${result.events.length} events (total: ${result.totalCount})`
      );
    }
  ...
  try {
    ...
    const bodyMaxPrice = isNumber(body.maxPrice)
      ...
      : isString(body.maxPrice)
        ...
        : undefined;
    const bodyTagsInclude = isStringArray(body.tagsInclude) ? body.tagsInclude : undefined;
    const bodyTagsExclude = isStringArray(body.tagsExclude) ? body.tagsExclude : undefined;
    const bodyLocations = isStringArray(body.locations) ? body.locations : undefined;
    const bodyZips = isStringArray(body.zips) ? body.zips : undefined;
    const bodyBlockedHosts = isStringArray(body.blockedHosts) ? body.blockedHosts : undefined;
    ...
    const bodyUseDefaultFilters = isBoolean(body.useDefaultFilters)
      ...
      : undefined;
    const bodyShowDailyEvents = isBoolean(body.showDailyEvents) ? body.showDailyEvents : undefined;
    const bodyIncludeMetadata = isBoolean(body.includeMetadata) ? body.includeMetadata : undefined;

    const params: EventFilterParams = {
      cursor: bodyCursor || searchParams.get('cursor') || undefined,
      limit: Math.min(bodyLimit ?? parseInt(searchParams.get('limit') || '50', 10), 100),
      search: bodySearch || searchParams.get('search') || undefined,
      dateFilter: bodyDateFilter || parseDateFilter(searchParams.get('dateFilter')) || undefined,
      ...
      priceFilter:
        bodyPriceFilter || parsePriceFilter(searchParams.get('priceFilter')) || undefined,
      maxPrice:
        bodyMaxPrice ??
      ...
      blockedHosts: bodyBlockedHosts || parseArray(searchParams.get('blockedHosts')),
