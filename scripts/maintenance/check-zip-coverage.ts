...
async function main() {
  ...
  const withZip = await db
    .select({ count: sql<number>`count(*)` })
    .from(events)
    .where(isNotNull(events.zip));
  const withoutZip = await db
    .select({ count: sql<number>`count(*)` })
    .from(events)
  ...
  console.log(
    `Coverage: ${Math.round((Number(withZip[0].count) / Number(total[0].count)) * 100)}%`
  );
  console.log('');
  console.log('By Source:');
  for (const row of bySource as unknown as {
    source: string;
    total: number;
    with_zip: number;
    without_zip: number;
