/**
 * Test Script for PechaKucha Night Asheville Scraper
 *
 * Runs the scraper in debug mode, saving raw data and validation
 * reports to a debug folder for inspection.
 *
 * Usage:
 *   npx tsx scripts/scrapers/test-pechakucha.ts          # Scraper test only
 *   npx tsx scripts/scrapers/test-pechakucha.ts --db     # Scraper + database test
 */
...
const DEBUG_DIR = path.join(process.cwd(), 'debug-scraper-pechakucha');
...
const SOURCE = 'PECHAKUCHA';

async function runScraperTest(): Promise<ScrapedEvent[]> {
  console.log('='.repeat(70));
  console.log('SCRAPER TEST - PECHAKUCHA NIGHT ASHEVILLE');
  console.log('='.repeat(70));
  console.log();
  ...
  const events = await scrapePechaKucha();
  ...
  if (events.length > 0) {
    ...
    console.log(
      `  With zips:         ${withZips}/${events.length} (${pct(withZips, events.length)}%)`
    );
    ...
    // All events (PechaKucha only has a few at a time)
...
async function main() {
  ...
  try {
    ...
    if (runDbTest) {
      ...
      console.log('  npx tsx scripts/scrapers/test-pechakucha.ts --db');
      console.log();
    }
  } catch (error) {
    console.error();
