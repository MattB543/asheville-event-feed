/**
 * Test Script for Story Parlor Scraper
 *
 * Runs the scraper in debug mode, saving raw data and validation
 * reports to a debug folder for inspection.
 *
 * Usage:
 *   npx tsx scripts/scrapers/test-storyparlor.ts          # Scraper test only
 *   npx tsx scripts/scrapers/test-storyparlor.ts --db     # Scraper + database test
 */
...
const DEBUG_DIR = path.join(process.cwd(), 'debug-scraper-storyparlor');
...
const SOURCE = 'STORY_PARLOR';

async function runScraperTest(): Promise<ScrapedEvent[]> {
  console.log('='.repeat(70));
  console.log('SCRAPER TEST - STORY PARLOR');
  console.log('='.repeat(70));
  console.log();
  ...
  const events = await scrapeStoryParlor();
...
async function main() {
  ...
  try {
    ...
    if (runDbTest) {
      ...
      console.log('  npx tsx scripts/scrapers/test-storyparlor.ts --db');
      console.log();
    }
  } catch (error) {
    console.error();
