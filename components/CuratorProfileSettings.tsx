...
export default function CuratorProfileSettings({ email, avatarUrl }: CuratorProfileSettingsProps) {
  ...
  useEffect(() => {
    void fetch('/api/curator/settings')
      .then((res) => res.json())
      .then((data: { profile?: CuratorProfile }) => {
        if (data.profile) {
          ...
          setShowProfilePicture(data.profile.showProfilePicture);
        } else {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as { profile?: CuratorProfile; error?: string };
      if (data.profile) {
        setProfile(data.profile);
  ...
  const copyProfileUrl = () => {
    if (profile?.slug) {
      void navigator.clipboard.writeText(`${window.location.origin}/u/${profile.slug}`);
      setCopied(true);
