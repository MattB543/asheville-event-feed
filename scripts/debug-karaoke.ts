...
async function check() {
  ...
  const samples = await db
    ...
    .where(
      and(
        sql`array_to_string(${events.tags}, ' ') ILIKE '%karaoke%'`,
  ...
  let standard = 0, quality = 0, outstanding = 0, noScore = 0;
  samples.forEach((e) => {
  ...
  samples.slice(0, 15).forEach((e) =>
    console.log(
      `- score=${e.score ?? "null"} | ${e.title.slice(0, 50)}`
    )
  );
