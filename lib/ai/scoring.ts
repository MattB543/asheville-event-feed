...
export async function generateEventScore(
  ...
  if (!isAzureAIEnabled()) {
    console.warn('[AI:Score] Azure AI not configured, skipping');
    return null;
  }
...
${similarEventsText}`;
  ...
  try {
    ...
    if (!result) {
      console.warn(`[AI:Score] No response from Azure AI for "${event.title.slice(0, 40)}..."`);
      return null;
    }
    ...
    const clamp = (n: unknown, label: string, min = 0): number => {
      ...
      if (result !== num) {
        clamped.push(`${label}: ${num}->${result}`);
      }
      return result;
    };

    const rarity = clamp(parsed.rarity, 'rarity');
    const unique = clamp(parsed.unique, 'unique');
    const magnitude = clamp(parsed.magnitude, 'magnitude');
    const score = rarity + unique + magnitude;

    // Secondary dimensions (1-10 scale)
    const ashevilleWeird = clamp(parsed.ashevilleWeird, 'ashevilleWeird', 1);
    const social = clamp(parsed.social, 'social', 1);

    if (clamped.length > 0) {
      console.warn(
        `[AI:Score] Clamped values for "${event.title.slice(0, 30)}...": ${clamped.join(', ')}`
      );
    }
    ...
    console.error(
      `[AI:Score] Error scoring "${event.title.slice(0, 40)}...":`,
