 ...
 *
 * Usage:
 *   npx tsx scripts/test-bmcmuseum.ts          # Scraper test only
 *   npx tsx scripts/test-bmcmuseum.ts --db     # Scraper + database test
 */
...
async function main() {
  ...
  try {
    ...
    if (runDbTest) {
      ...
      console.log('  npx tsx scripts/test-bmcmuseum.ts --db');
      console.log();
    }
  } catch (error) {
    console.error();
