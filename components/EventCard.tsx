...
interface EventCardProps {
  ...
  onSignalCapture?: (
    eventId: string,
    signalType: "calendar" | "share" | "viewSource"
  ) => void;
...
export default function EventCard({
  ...
  const eventUrl = `/events/${generateEventSlug(
    event.title,
    event.startDate,
    event.id
  )}`;
  ...
  const locationText = event.organizer && event.location
    ...
    <div>
      ...
      >
        ...
        <Link href={eventUrl} onClick={(e) => e.stopPropagation()} className="block">
          <div className={`relative w-full ${isMobileExpanded ? "h-48" : "h-[130px]"} bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden transition-all duration-200 shadow-sm`}>
        ...
        <div className="pt-3">
          ...
          {cleanedAiSummary && (
            <p className={`text-sm text-gray-600 dark:text-gray-300 leading-relaxed mt-2 ${!isMobileExpanded ? "line-clamp-2" : ""}`}>
              {cleanedAiSummary}
            </p>
          )}

          {/* Badges: Price, Date/Time, Tags */}
            ...
            <div className="flex flex-wrap gap-2 mt-4" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="relative" ref={calendarMenuRef}>
                ...
                >
                  ...
                  <span>Calendar</span>
                  <ChevronDown size={12} className={`transition-transform ${calendarMenuOpen ? "rotate-180" : ""}`} />
                </button>
              ...
              >
                <Heart size={14} className={`transition-transform ${isFavorited ? "fill-current" : ""} ${isHeartAnimating ? "animate-heart-pop" : ""}`} />
              ...
              <div className="relative">
                <button
                  onClick={async () => {
                    const shareUrl = `${window.location.origin}/events/${generateEventSlug(event.title, event.startDate, event.id)}`;
                  ...
                  <div className="absolute right-0 top-full mt-1 z-50 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-lg min-w-[200px]">
                    <button
                      ...
                      className={`w-full flex items-center gap-2 px-3 py-2 text-xs transition-colors cursor-pointer ${
                        isLoggedIn ? "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" : "text-gray-400 dark:text-gray-500"
                      }`}
                    >
                      <Bookmark size={14} className={isCurated ? "fill-current text-brand-600" : ""} />
                      {isLoggedIn ? (isCurated ? "Remove from profile" : "Curate") : "Curate - log in to use"}
                    </button>
                    <a
                      href={eventUrl}
                      target="_blank"
                    ...
                    <button
                      ...
                      className={`w-full flex items-start gap-2 px-3 py-2 text-left cursor-pointer ${
                        isNewlyHidden ? "text-gray-400 dark:text-gray-600" : "text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
                      }`}
                    >
                    ...
                    {event.organizer && (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-gray-500 dark:text-gray-400">Hide all events from this host</div>
                        </div>
                      </button>
                    )}
                    <div className="border-t border-gray-200 dark:border-gray-700" />
      ...
      {displayMode === "minimized" && (
        ...
        >
          ...
          <ChevronDown
            size={14}
            className="text-gray-400 dark:text-gray-500 opacity-60 shrink-0"
          />
        </div>
      )}
