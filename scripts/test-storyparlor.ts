 ...
 *
 * Usage:
 *   npx tsx scripts/test-storyparlor.ts          # Scraper test only
 *   npx tsx scripts/test-storyparlor.ts --db     # Scraper + database test
 */
...
async function main() {
  ...
  try {
    ...
    if (runDbTest) {
      ...
      console.log('  npx tsx scripts/test-storyparlor.ts --db');
      console.log();
    }
  } catch (error) {
    console.error();
