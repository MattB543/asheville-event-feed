...
export async function GET() {
  try {
    ...
    const positiveEvents: TasteEvent[] = positiveSignals.reduce<TasteEvent[]>((acc, signal) => {
    ...
    const negativeEvents: TasteEvent[] = negativeSignals.reduce<TasteEvent[]>((acc, signal) => {
      const event = eventMap.get(signal.eventId);
      if (!event) return acc;
      acc.push({
        event: {
