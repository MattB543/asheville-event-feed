...
export interface TomorrowEmailConfig {
  subject: string;
  templatePath: string;
...
export type TomorrowEmailBuilder = (options: {
  recipientName?: string | null;
  status: string;
}) => Promise<{ subject: string; htmlBody: string; textBody: string }>;

export function createTomorrowEmailBuilder(config: TomorrowEmailConfig): TomorrowEmailBuilder {
  const { subject, templatePath, draftOnlyLine, headerTitle } = config;
  let templatePromise: Promise<string> | null = null;

  async function loadTemplate(): Promise<string> {
    if (!templatePromise) {
      templatePromise = readFile(templatePath, 'utf-8').then((content) => content.trim());
    }

    const template = await templatePromise;
    if (!template.includes(FIRST_NAME_TOKEN)) {
      throw new Error(`Template is missing first-name token: ${templatePath}`);
    }
  ...
  function buildTextBody(template: string, recipientName: string, status: string): string {
    ...
    const lines = template
      ...
      .split(/\r?\n/)
      .filter((line) => (status === 'draft' ? true : line.trim() !== draftOnlyLine))
      .join('\n')
      .replace(/\n{3,}/g, '\n\n')
      .trim();
  ...
  return async function buildTomorrowEmail(options: {
    recipientName?: string | null;
    status: string;
  }): Promise<{ subject: string; htmlBody: string; textBody: string }> {
