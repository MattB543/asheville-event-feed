...
export const events = pgTable(
  ...
  (table) => ({
    ...
    embeddingIdx: index('events_embedding_idx').using(
      'hnsw',
...
export const submittedEvents = pgTable(
  'submitted_events',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const curatedEvents = pgTable(
  'curated_events',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    userId: uuid('user_id')
      .notNull()
      .references(() => curatorProfiles.userId, { onDelete: 'cascade' }),
  ...
  (table) => ({
    userEventUnique: uniqueIndex('curated_events_user_event_unique').on(
      table.userId,
      table.eventId
    ),
