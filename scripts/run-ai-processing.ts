...
// Configuration
const BATCH_SIZE_COMBINED = 5;
const BATCH_SIZE_EMBEDDINGS = 10;
const LIMIT_PER_PASS = 100; // Process 100 events per pass
  ...
  for (const event of eventsNeedingScores) {
    try {
      ...
      if (scoreResult) {
        ...
        // Only log high scores
        if (scoreResult.score >= 15) {
