...
async function main() {
  ...
  console.log(
    `Found ${untaggedEvents.length} events needing tags (out of ${allEvents.length} total).`
  );
  ...
  for (const [index, event] of untaggedEvents.entries()) {
    console.log(
      `[${index + 1}/${untaggedEvents.length}] Processing event: "${event.title}" (ID: ${event.id})`
    );
    ...
    try {
      ...
      if (tags.length > 0) {
        await db.update(events).set({ tags }).where(eq(events.id, event.id));
